...
namespace Shader
{
    ...
    namespace NormalDistributionShader
    {
        ...
        inline float ND_GGX(const float &NdotH, const float &alpha)
        {
            ...
            return roughness_2 / std::max(frac_below, EPSILON);
        }

        // GTR法线分布函数，即Generalized-Trowbridge-Reitz
        inline float ND_GTR(const float &NdotH, const float &alpha, const int gamma)
        {
            // gamma控制高光尾部效果
